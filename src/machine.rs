...
impl Machine {
    ...
    fn run<L, N>(
        ...
        yield_fn: &mut impl FnMut(&Self) -> Result,
    ) -> Result
    where
        L: Language,
        N: Analysis<L>,
            ...
            match instruction {
                Instruction::Bind { i, out, node } => {
                    ...
                    return for_each_matching_node(&egraph[self.reg(*i)], node, |matched| {
                        ...
                        self.run(egraph, remaining_instructions, yield_fn)
                    });
                }
...
impl Machine {
    pub fn run_program_collect_with_limit<L, N>(
        &mut self,
        egraph: &EGraph<L, N>,
        program: &Program<L>,
        eclass: Id,
        ...
        N: Analysis<L>,
    {
        ...
        self.run_program(egraph, program, eclass, |subst| {
            if limit == 0 {
                Err(())
            } else {
    ...
    pub fn run_program<L, N>(
        &mut self,
        egraph: &EGraph<L, N>,
        program: &Program<L>,
        eclass: Id,
        mut f: impl FnMut(&Subst) -> Result,
    ) -> Result
    where
        L: Language,
        N: Analysis<L>,
    {
        ...
        self.reg.clear();
        self.lookup.clear();
        self.reg.push(eclass);

        let mut template = program.subst.clone();

        self.run(egraph, &program.instructions, &mut |machine| {
            ...
            for ((v1, reg), (v2, id)) in program.subst.vec.iter().zip(template.vec.iter_mut()) {
                debug_assert_eq!(v1, v2);
                // HACK we are reusing Ids here, this is bad
                *id = machine.reg(Reg(usize::from(*reg) as u32));
            }
